	...
	<div class="container mobileDiv py-5">
		...
		<div class="row">
			<div class="col text-center">				
				<h2 class="">You Are Editing {{ $guest->name }}'s Invitation</h2>
			</div>
		</div>
		...
		<div class="row">
			...
			<div class="col">
				...
				{!! Form::model($guest, [ 'action' => ['GuestController@update2', $guest->id], 'method' => 'PATCH', 'class' => 'editGuestForm']) !!}
					...
					<div class="form-row align-items-center form-row justify-content-center">
						
						<div class="md-form col-6">

							<input id="first" class="form-control" type="text" name="name" value="{{ $guest->name }}" />
					...
					<div class="form-row align-items-center form-row justify-content-center">
						...
						<div class="md-form col-6">
							...
							<div class="d-flex align-items-center justify-content-center">
								<button class="inviteCheck btn{{ $guest->plusOne->rsvp == 'Y' ? ' green active' : ' stylish-color-dark' }}"  id="rsvpYes" type="button">Comfirmed Invite
									<input hidden class="hidden" type="checkbox" name="plus_one_rsvpYes" value="Y"{{ $guest->plusOne->rsvp == "Y" ? ' checked' : '' }} />
								</button>


								<button class="inviteCheck btn{{ $guest->plusOne->rsvp == 'N' ? ' red active' : ' stylish-color-dark' }}" id="rsvpNo" type="button">Declined Invite
									<input hidden class="hidden" type="checkbox" name="plus_one_rsvpNo" value="Y"{{ $guest->plusOne->rsvp == "N" ? ' checked' : '' }} />
								</button>
							</div>
